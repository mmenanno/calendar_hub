...
module CalendarHub
  class FilterSyncServiceTest < ActiveSupport::TestCase
    def setup
      ...
      @service = FilterSyncService.new(source: @source, apple_client: @apple_client_mock)
      @service_with_default_client = FilterSyncService.new(source: @source)
    end
    ...
    test "initialize uses default AppleCalendar::Client when not provided" do
      ...
      service = FilterSyncService.new(source: @source)
    ...
    test "sync_filter_rules returns zeros when source is blank" do
      service = FilterSyncService.new(source: nil, apple_client: @apple_client_mock)
    ...
    test "sync_event_filter_status upserts event when not sync_exempt" do
      ...
      translator_mock = mock("EventTranslator")
      payload = {
        uid: "ch-#{@source.id}-prov-123",
        summary: "Initial Consultation",
        description: "Consultation session",
      }
      translator_mock.expects(:call).with(event).returns(payload)
      CalendarHub::Translators::EventTranslator.expects(:new).with(@source).returns(translator_mock)

      mapped_title = "Mapped: Initial Consultation"
      CalendarHub::NameMapper.expects(:apply).with("Initial Consultation", source: @source).returns(mapped_title)
      ...
      expected_payload = payload.merge(
        summary: mapped_title,
        url: event_url,
        x_props: { "X-CH-SOURCE" => @source.name, "X-CH-SOURCE-ID" => @source.id.to_s },
      )

      @apple_client_mock.expects(:upsert_event).with(
        calendar_identifier: @source.calendar_identifier,
    ...
    test "sync_event_filter_status handles StandardError during upsert and re-raises" do
      ...
      translator_mock = mock("EventTranslator")
      payload = { uid: "ch-#{@source.id}-prov-123", summary: "Initial Consultation" }
      translator_mock.expects(:call).with(event).returns(payload)
      CalendarHub::Translators::EventTranslator.expects(:new).with(@source).returns(translator_mock)

      CalendarHub::NameMapper.expects(:apply).with("Initial Consultation", source: @source).returns("Mapped Title")
      ...
      Rails.application.routes.url_helpers.expects(:calendar_event_url).with(
        event,
        **url_options,
      ).returns("https://example.com/event")
